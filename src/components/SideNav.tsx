...
export default function SideNav({ activeBar, setActiveBar }: Props) {
    const { user } = useUser();
    ...
    return (
        <menu role="menu" className={`w-[28%] h-full font-taruno border-r ${theme === 'dark' ? 'bg-black border-r-tr-white' : 'bg-dk-white border-r-tr-gray'}`}>
            <header className="border-b border-b-tr-white py-4 m-4">
                <h3 className={`text-base text-center  ${theme === 'dark' ? '' : ''}`}>
                    Hello <span className={theme === 'dark' ? 'text-gold' : 'text-gold'}>{user?.username}</span>
                </h3>
            </header>

            <section className="relative w-full mt-16 px-6 flex flex-col items-start gap-5">
